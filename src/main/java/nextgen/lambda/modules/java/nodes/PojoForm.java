            ...
            fieldForm.rows().forEach(row -> {
               row.getTextField("name").flatMap(nextgen.lambda.ui.components.StringComponent::value).ifPresent(names -> {
                  ...
                  for (String item : ss) {
                     ...
                     final nextgen.lambda.modules.templates.java.classFieldBuilder builder = nextgen.lambda.modules.templates.java.JavaGroup.new_classFieldBuilder()
                        .set_scope("public")
                        .set_name(s);
                     ...
                     row
                        ...
                        .ifPresent(value -> {
                           row.getButton("init").flatMap(nextgen.lambda.ui.components.ButtonComponent::value).map(Object::toString).ifPresent(builder::set_init);
                           ...
                           builder.set_type(fieldType);
                        });

                     declarations.add(getJavaGroup().render_classField(builder));
                  }
               });
            });
